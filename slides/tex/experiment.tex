
\section{Experiment}

\subsection{}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
{
%\paper{Xochicale et al. 2018 in Progress}

\begin{frame}{Human-Humanoid Imitation Activities}
    \begin{figure}
        \includegraphicscopyright[width=0.8\linewidth]{experiment/hri}{Work in progress (Xochicale M. et al. 2018)}
	\caption[PA]{(A) Front-to-Front Human-Humanoid Imitation 
		of Horizontal Movements,
		(B) NAO, humanoid robot, and
		(C) 20 rigth-handed healthy participants (age: mean=19.8, SD=1.39).
		}
   \end{figure}
	
\end{frame}
}

\subsection{}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
{
%\paper{Xochicale et al. 2018 in Progress}

\begin{frame}{Horizontal Arm Movements}
    \begin{figure}
        \includegraphicscopyright[width=1.0\linewidth]{experiment/hri_armspeed}{Work in progress (Xochicale M. et al. 2018)}
	\caption[PA]{
		Arm Movements for two speed conditions:
		 Horizontal Normal (HN) and Horizontal Faster (HF) 
		with different window lengths.
		}
   \end{figure}
	
\end{frame}
}




